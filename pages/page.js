import React from 'react'
import Link from 'next/link'
import Head from 'next/head'

import Navbar from '../components/navbar'
import Footer1 from '../components/footer1'

const Page = (props) => {
  return (
    <>
      <div className="page-container">
        <Head>
          <title>Page - Danneskiold & Associates | Law Firm</title>
          <meta
            name="description"
            content="Danneskiold &amp; Associates is a boutique law firm in Bangkok, providing expertise in criminal, immigration, tort, contract, and corporate law."
          />
          <meta
            property="og:title"
            content="Page - Danneskiold &amp; Associates | Law Firm "
          />
          <meta
            property="og:description"
            content="Danneskiold &amp; Associates is a boutique law firm in Bangkok, providing expertise in criminal, immigration, tort, contract, and corporate law."
          />
          <meta
            property="og:image"
            content="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/1e7f4ee4-0cee-4fac-a5b5-9bf26476dc3e/5631a551-dd96-44ef-935a-e149f868456e?org_if_sml=1&amp;force_format=original"
          />
        </Head>
        <Navbar rootClassName="navbar-root-class-name12"></Navbar>
        <div className="page-blog-post-header3 thq-section-padding">
          <div className="page-max-width thq-section-max-width">
            <div className="page-breadcrumbs">
              <span className="page-category thq-body-small">Criminal Law</span>
            </div>
            <div className="page-content thq-flex-column">
              <h1 className="page-title thq-heading-1">
                การดำเนินคดีอาญาในประเทศไทย
              </h1>
              <span className="page-read-time thq-body-small">5 min read</span>
              <div className="page-content1">
                <img
                  alt="attorney manita sroykeeree"
                  src="/photo_2024-04-29_08-16-34-1400w.jpg"
                  className="page-author-image thq-img-round thq-img-ratio-1-1"
                />
                <div className="page-author">
                  <span className="page-text thq-body-small">Written by</span>
                  <span className="page-text01 thq-body-small">Manita</span>
                </div>
                <div className="page-time">
                  <span className="page-date thq-body-small">4 May 2024</span>
                </div>
                <div className="page-container1">
                  <Link href="/criminal-proceedings">
                    <a className="page-link">
                      <img
                        alt="image"
                        src="/united-kingdom-200h.png"
                        className="page-image"
                      />
                    </a>
                  </Link>
                  <Link href="/page">
                    <a className="page-link1">
                      <img
                        alt="image"
                        src="/thailand-200h.png"
                        className="page-image1"
                      />
                    </a>
                  </Link>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="page-container2">
          <span className="page-text02">การดำเนินคดีอาญาในประเทศไทย</span>
          <span className="page-text03">
            <span className="page-text04">                 </span>
            <br className="page-text05"></br>
            <br className="page-text06"></br>
            <span className="page-text07">
                   
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text08"> </span>
            <span className="page-text09">
              ตามประมวลกฎหมายวิธีพิจารณาความอาญา มาตรา 28
              ได้กำหนดให้บุคคลที่มีอำนาจฟ้องคดีอาญาได้แก่
            </span>
            <span className="page-text10">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text11">พนักงานอัยการและผู้เสียหาย</span>
            <span className="page-text12">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text13">
              จะเห็นได้ว่าตามกฎหมายไทยสามารถดำเนินคดีอาญาได้โดยรัฐ (State
              Prosecution) และดำเนินคดีอาญาได้โดยเอกชน (Private Prosecution)
              กล่าวคือ
              กฎหมายให้สิทธิแก่ผู้เสียหายในการฟ้องคดีอาญาได้เช่นเดียวกับพนักงานอัยการ
              และอำนาจในการฟ้องคดีอาญาของพนักงานอัยการและผู้เสียหายนั้น
              มีความเป็นอิสระแยกต่างหากออกจากกัน
            </span>
            <br className="page-text14"></br>
            <br className="page-text15"></br>
            <span className="page-text16">
              การดำเนินคดีอาญาโดยเอกชน (ผู้เสียหาย)
            </span>
            <span className="page-text17">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text18"></br>
            <br className="page-text19"></br>
            <span className="page-text20">
                   
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text21">
              ตามกฎหมายไทย
              ผู้เสียหายมีอำนาจฟ้องคดีอาญาได้เท่าเทียมกับพนักงานอัยการ
              โดยไม่ต้องคำนึงว่าคดีนั้นจะได้มีการร้องทุกข์ต่อตำรวจหรือมีการสอบสวนของพนักงานสอบสวนหรือไม่
              โดยผู้เสียหายสามารถแต่งตั้งทนายความยื่นคำฟ้องคดีอาญาต่อศาลที่มีเขตอำนาจได้
            </span>
            <span className="page-text22">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text23"></br>
            <br className="page-text24"></br>
            <span className="page-text25">     </span>
            <span className="page-text26">
              {' '}
              โดยการฟ้องคดีอาญาโดยผู้เสียหายนั้นกฎหมายกำหนดให้
            </span>
            <span className="page-text27">ศาลต้องทำการไต่สวนมูลฟ้อง</span>
            <span className="page-text28">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text29">
              (กระบวนการที่ศาลทำการตรวจสอบข้อเท็จจริงในคดีที่ฟ้องว่ามีมูลเพียงพอที่ศาลจะรับฟ้องไว้พิจารณาต่อไปหรือไม่)
              ซึ่งแตกต่างจากคดีอาญาที่พนักงานอัยการเป็นผู้ฟ้องคดี
              (การดำเนินคดีอาญาโดยรัฐ) ซึ่งศาลไม่จำต้องไต่สวนมูลฟ้องก่อน
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text30"></br>
            <br className="page-text31"></br>
            <span className="page-text32">
                    ในกรณีที่ผู้เสียหายเป็นผู้เยาว์หรือผู้ไร้ความสามารถ
              ให้ผู้แทนโดยชอบธรรม (บิดา มารดา ผู้ปกครองหรือผู้รับบุตรบุญธรรม)
              ของผู้เยาว์หรือผู้อนุบาลของผู้ไร้ความสามารถเป็นผู้มีอำนาจดำเนินคดีอาญาแทนผู้เสียหาย
            </span>
            <br className="page-text33"></br>
            <br className="page-text34"></br>
            <span className="page-text35">
                   
              ในกรณีที่ผู้เสียหายถึงแก่ความตายหรือได้รับบาดเจ็บถึงขนาดไม่สามารถดำเนินคดีเองได้
              ผู้บุพการี ผู้สืบสันดาน
              สามีหรือภริยาของผู้เสียหายมีอำนาจดำเนินคดีอาญาแทนผู้เสียหายได้
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text36"></br>
            <br className="page-text37"></br>
            <span className="page-text38">
                   
              ในกรณีที่ผู้เสียหายได้ยื่นฟ้องคดีอาญาไว้แล้วต่อมาเสียชีวิตในขณะที่คดีอาญากำลังอยู่ในระหว่างการพิจารณา
              ผู้บุพการี ผู้สืบสันดาน
              สามีหรือภริยาของผู้เสียหายที่เสียชีวิตสามารถดำเนินคดีแทนต่อไปโดยยื่นคำร้องขอเข้ามาในคดีเพื่อดำเนินคดีต่างผู้ตายได้
            </span>
            <span className="page-text39">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text40"></br>
            <br className="page-text41"></br>
            <span className="page-text42">
              การดำเนินคดีอาญาโดยรัฐ (พนักงานอัยการ)
            </span>
            <br className="page-text43"></br>
            <br className="page-text44"></br>
            <span className="page-text45">
                   
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text46">
              หากผู้เสียหายต้องการดำเนินคดีอาญาโดยพนักงานอัยการ ผู้เสียหายจะต้อง
            </span>
            <span className="page-text47">ดำเนินการร้องทุกข์</span>
            <span className="page-text48">
              ต่อสถานีตำรวจและพนักงานสอบสวนต้องทำการสอบสวนก่อน
              เพราะในการดำเนินคดีอาญาโดยรัฐนั้นกฎหมายห้ามไม่ให้พนักงานอัยการฟ้องคดี
              หากคดีนั้นไม่ได้มีการสอบสวนมาก่อน
              (แตกต่างจากการฟ้องคดีอาญาโดยผู้เสียหายที่ไม่จำต้องมีการสอบสวนมาก่อน)
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text49"></br>
            <br className="page-text50"></br>
            <span className="page-text51">
                    ในกรณีที่มีการสอบสวนในความผิดนั้นแล้ว
              พนักงานอัยการจะคำสั่งฟ้องคดีนั้นหรือไม่ก็ได้
              หากพนักงานอัยการมีคำสั่งฟ้องคดี
              พนักงานอัยการก็จะดำเนินคดีนั้นต่อไป
              (ซึ่งผู้เสียหายสามารถยื่นคำร้องขอเข้าเป็นโจทก์ร่วมกับพนักงานอัยการได้)
              หากพนักงานอัยการมีคำสั่งไม่ฟ้องคดี
              ย่อมไม่ตัดสิทธิผู้เสียหายที่จะฟ้องคดีอาญาต่อศาลเอง
            </span>
            <span className="page-text52">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text53"></br>
            <br className="page-text54"></br>
            <span className="page-text55">หมายเหตุ:</span>
            <br className="page-text56"></br>
            <br className="page-text57"></br>
            <span className="page-text58">
                   
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text59">
              ในคดีอาญาความผิดต่อส่วนตัวหรือความผิดอันยอมความได้
            </span>
            <span className="page-text60">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text61">
              กล่าวคือ ความผิดที่มีผลกระทบต่อผู้ที่ถูกกระทำโดยตรงเท่านั้น
              มิได้ก่อให้เกิดความเสียหายแก่รัฐหรือสังคม เช่น
              ความผิดฐานหมิ่นประมาท ฉ้อโกง ยักยอกทรัพย์ ทำให้เสียทรัพย์ อนาจาร
              ข่มขืนกระทำชำเรา เป็นต้น
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <br className="page-text62"></br>
            <br className="page-text63"></br>
            <span className="page-text64">
                    หากผู้เสียหายดำเนินคดีอาญาด้วยตนเอง
              ผู้เสียหายจะต้องฟ้องคดีต่อศาลภายใน 3 เดือน นับแต่วันที่
              “รู้เรื่องความผิด” และ “รู้ตัวผู้กระทำความผิด”
              โดยผู้เสียหายไม่ต้องร้องทุกข์ต่อสถานีตำรวจ
              แต่หากผู้เสียหายต้องการดำเนินคดีอาญาโดยพนักงานอัยการ
              ผู้เสียหายจะต้องไปร้องทุกข์ภายในกำหนดเวลา 3
              เดือนนับแต่รู้ถึงความผิดและรู้ตัวผู้กระทำผิดเช่นกัน
            </span>
            <span className="page-text65">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <span className="page-text66">
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
          </span>
        </div>
        <div className="page-container3">
          <Link href="/contact">
            <a className="page-link2">
              <h1 className="page-text67">Contact</h1>
            </a>
          </Link>
        </div>
        <Footer1 rootClassName="footer1-root-class-name1"></Footer1>
      </div>
      <style jsx>
        {`
          .page-container {
            width: 100%;
            display: flex;
            overflow: auto;
            min-height: 100vh;
            align-items: center;
            flex-direction: column;
            background-color: var(--dl-color-theme-neutral-light);
          }
          .page-blog-post-header3 {
            display: flex;
            position: relative;
            align-items: center;
          }
          .page-max-width {
            gap: var(--dl-space-space-oneandhalfunits);
            display: flex;
            align-items: center;
            flex-direction: column;
          }
          .page-breadcrumbs {
            gap: var(--dl-space-space-halfunit);
            display: flex;
            align-items: center;
          }
          .page-category {
            font-size: 1.2rem;
            font-style: normal;
            text-align: center;
            font-family: 'Josefin Sans';
            font-weight: 300;
          }
          .page-content {
            gap: var(--dl-space-space-oneandhalfunits);
            display: flex;
            align-items: center;
            flex-direction: column;
          }
          .page-title {
            font-size: 2.5rem;
            font-style: normal;
            text-align: center;
            font-family: 'Noto Sans Thai';
            font-weight: 400;
          }
          .page-read-time {
            font-size: 1.2rem;
            font-style: normal;
            text-align: center;
            font-family: 'Josefin Sans';
            font-weight: 300;
          }
          .page-content1 {
            display: flex;
            align-items: center;
            flex-direction: column;
          }
          .page-author-image {
            width: var(--dl-size-size-small);
            height: var(--dl-size-size-small);
          }
          .page-author {
            gap: var(--dl-space-space-halfunit);
            display: flex;
            margin-top: var(--dl-space-space-halfunit);
          }
          .page-text {
            font-size: 1.2rem;
            font-style: normal;
            font-family: 'Josefin Sans';
            font-weight: 200;
          }
          .page-text01 {
            font-size: 1.2rem;
            font-style: normal;
            font-family: 'Josefin Sans';
            font-weight: 300;
          }
          .page-time {
            gap: 8px;
            display: flex;
            align-items: center;
          }
          .page-date {
            font-size: 1.2rem;
            font-style: normal;
            font-family: 'Josefin Sans';
            font-weight: 200;
          }
          .page-container1 {
            flex: 0 0 auto;
            width: 100%;
            height: auto;
            display: flex;
            align-items: center;
            justify-content: center;
          }
          .page-link {
            display: contents;
          }
          .page-image {
            width: 32px;
            height: 32px;
            object-fit: cover;
            margin-left: var(--dl-space-space-unit);
            text-decoration: none;
          }
          .page-link1 {
            display: contents;
          }
          .page-image1 {
            width: 32px;
            height: 32px;
            object-fit: cover;
            margin-left: var(--dl-space-space-unit);
            margin-right: var(--dl-space-space-unit);
            text-decoration: none;
          }
          .page-container2 {
            flex: 0 0 auto;
            width: 75%;
            height: auto;
            display: flex;
            position: relative;
            align-items: center;
            flex-direction: column;
            justify-content: center;
          }
          .page-text02 {
            font-size: 1.5rem;
            font-style: normal;
            text-align: center;
            font-family: 'Noto Sans Thai';
            font-weight: 600;
          }
          .page-text03 {
            width: 60%;
            font-size: 1.5rem;
            font-family: 'Noto Sans Thai';
            font-weight: 400;
          }
          .page-text04 {
            font-style: normal;
          }
          .page-text05 {
            font-style: normal;
          }
          .page-text06 {
            font-style: normal;
          }
          .page-text07 {
            font-style: normal;
          }
          .page-text08 {
            font-weight: 300;
          }
          .page-text09 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text10 {
            font-style: normal;
          }
          .page-text11 {
            font-style: normal;
            font-weight: 400;
          }
          .page-text12 {
            font-style: normal;
          }
          .page-text13 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text14 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 200;
          }
          .page-text15 {
            font-style: normal;
          }
          .page-text16 {
            font-style: normal;
            font-weight: 400;
          }
          .page-text17 {
            font-weight: 600;
          }
          .page-text18 {
            font-size: 1.5rem;
            font-family: 'Noto Sans Thai';
            font-weight: 600;
          }
          .page-text19 {
            font-style: normal;
          }
          .page-text20 {
            font-style: normal;
          }
          .page-text21 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text22 {
            font-weight: 300;
          }
          .page-text23 {
            font-size: 1.5rem;
            font-family: 'Noto Sans Thai';
            font-weight: 300;
          }
          .page-text24 {
            font-weight: 300;
          }
          .page-text25 {
            font-weight: 300;
          }
          .page-text26 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text27 {
            font-style: normal;
            font-weight: 400;
          }
          .page-text28 {
            font-style: normal;
          }
          .page-text29 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text30 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 200;
          }
          .page-text31 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text32 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text33 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 200;
          }
          .page-text34 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text35 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text36 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 200;
          }
          .page-text37 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text38 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text39 {
            font-style: normal;
          }
          .page-text40 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 400;
          }
          .page-text41 {
            font-style: normal;
          }
          .page-text42 {
            font-weight: 400;
          }
          .page-text43 {
            font-size: 1.5rem;
            font-family: 'Noto Sans Thai';
            font-weight: 400;
          }
          .page-text44 {
            font-style: normal;
          }
          .page-text45 {
            font-style: normal;
          }
          .page-text46 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text47 {
            font-style: normal;
            font-weight: 400;
          }
          .page-text48 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text49 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 200;
          }
          .page-text50 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text51 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text52 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text53 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 200;
          }
          .page-text54 {
            font-style: normal;
          }
          .page-text55 {
            font-style: normal;
            font-weight: 400;
          }
          .page-text56 {
            font-size: 1.5rem;
            font-family: 'Noto Sans Thai';
            font-weight: 600;
          }
          .page-text57 {
            font-style: normal;
          }
          .page-text58 {
            font-weight: 600;
          }
          .page-text59 {
            font-style: normal;
            font-weight: 400;
          }
          .page-text60 {
            font-style: normal;
          }
          .page-text61 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text62 {
            font-size: 1.5rem;
            font-style: normal;
            font-family: 'Noto Sans Thai';
            font-weight: 200;
          }
          .page-text63 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text64 {
            font-style: normal;
            font-weight: 200;
          }
          .page-text65 {
            font-style: normal;
          }
          .page-text66 {
            font-style: normal;
          }
          .page-container3 {
            flex: 0 0 auto;
            width: 100%;
            height: 100px;
            display: flex;
            position: relative;
            margin-top: var(--dl-space-space-fiveunits);
            align-items: center;
            margin-bottom: var(--dl-space-space-fiveunits);
            justify-content: center;
          }
          .page-link2 {
            display: contents;
          }
          .page-text67 {
            color: var(--dl-color-gray-black);
            padding: var(--dl-space-space-halfunit);
            font-size: 1.5rem;
            font-style: normal;
            transition: 0.3s;
            font-family: Josefin Sans;
            font-weight: 200;
            padding-top: var(--dl-space-space-unit);
            border-color: var(--dl-color-gray-black);
            border-width: 1px;
            padding-left: var(--dl-space-space-unit);
            padding-right: var(--dl-space-space-unit);
            padding-bottom: var(--dl-space-space-unit);
            text-decoration: none;
          }
          .page-text67:hover {
            color: var(--dl-color-gray-white);
            background-color: #1b1b1b;
          }
          @media (max-width: 1600px) {
            .page-text03 {
              width: 70%;
              font-size: 1.5rem;
            }
            .page-text04 {
              font-style: normal;
              font-weight: 700;
            }
          }
          @media (max-width: 1200px) {
            .page-text03 {
              width: 80%;
              font-size: 1.5rem;
            }
            .page-text04 {
              font-style: normal;
              font-weight: 700;
            }
          }
          @media (max-width: 991px) {
            .page-container2 {
              width: 80%;
            }
          }
          @media (max-width: 767px) {
            .page-container2 {
              width: 85%;
            }
            .page-text03 {
              width: 90%;
            }
            .page-text67 {
              align-self: flex-start;
            }
          }
          @media (max-width: 479px) {
            .page-container2 {
              width: 90%;
              margin-top: var(--dl-space-space-twounits);
            }
            .page-text03 {
              width: 90%;
            }
            .page-text67 {
              align-self: flex-start;
            }
          }
        `}
      </style>
    </>
  )
}

export default Page
